...
class VerisureApi {
	...
	sendRequest ( options ) {
		...
		return new Promise( function ( resolve, reject ) {
			request( options, function requestCallback( error, response, body ) {
				...
				} else if(response.headers['content-type'] !== 'application/json' || response.headers['content-type'] !== 'application/xml') {
					console.log('Received wrong content-type!');
					this.serverSelect();
					reject ('Wrong content-type');
				} else if(body["response"]["status"]) {
					this.serverSelect();
					reject ('Wrong content-type');
				}
	...
	authenticate() {
		...
		if(this._BASE_HOST === null){
			console.log('Base host was NULL');
			this.serverSelect();
		}
